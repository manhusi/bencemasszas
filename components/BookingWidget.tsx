...
import { fetchAvailability, createBooking, BookingApiError } from './bookingApi';
import {
    Slot,
...
const PUBLIC_API_KEY = 'ylBb8z7dhcK1zffhBpGDA2YTQl1dKNvB8FnPLyDqNFEVNtbG';
const BOOKING_API_KEY = 's84kM4JFKfYjSAnm0aehHabLSyCS5yl0lwAAgqKnz3KSJbh3';
...
export function BookingWidget() {
    ...
    useEffect(() => {
        const loadAvailability = async () => {
            try {
                setLoading(true);
                setError(null);
                const data = await fetchAvailability(BASE_URL, PUBLIC_API_KEY);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await createBooking(BASE_URL, BOOKING_API_KEY, {
                name: formData.name,
    ...
    const handleReset = () => {
        ...
        // Reload availability
        fetchAvailability(BASE_URL, PUBLIC_API_KEY)
            .then((data) => {
                setAvailability(data);
                if (data.service_id.length === 1) {
                    setSelectedService(data.service_id[0]);
